    ...
    internal sealed partial class ObjectPoolManager : ModuleImp, IObjectPoolManager
    {
        ...
        private sealed class ObjectPool<T> : ObjectPoolBase, IObjectPool<T> where T : ObjectBase
        {
            ...
            public void Release(int toReleaseCount, ReleaseObjectFilterCallback<T> releaseObjectFilterCallback)
            {
                ...
                _autoReleaseTime = 0f;
                //获取可以被释放的对象，缓存到_cachedCanReleaseObjects
                GetCanReleaseObjects(_cachedCanReleaseObjects);
                //基于一定的比较规则，筛选出最终的可释放对象列表toReleaseObjects（本质就是调用DefaultReleaseObjectFilterCallback函数）
            ...
            /// 淘汰列表筛选函数
            /// </summary>
            /// <param name="candidateObjects">候选列表</param>
            /// <param name="toReleaseCount">期望释放个数</param>
            /// <param name="expireTime">过期时间戳，最后一次使用时刻在此之前的对象均为过期对象</param>
            /// <returns></returns>
            private List<T> DefaultReleaseObjectFilterCallback(List<T> candidateObjects, int toReleaseCount, DateTime expireTime)
            {
                _cachedToReleaseObjects.Clear();
                
                //根据过期时间淘汰（这里不难发现，即使期望释放个数为1，但是也可能多释放过期资源）
                if (expireTime > DateTime.MinValue)
                {
                    for (int i = candidateObjects.Count - 1; i >= 0; i--)
                    {
                        //所有最后一次使用时间在过期时间之前的对象，加入淘汰列表
                ...
                //若考虑过期时间后仍达不到释放预期，还需要继续淘汰，根据优先级（和最后更新时间）冒泡排序
