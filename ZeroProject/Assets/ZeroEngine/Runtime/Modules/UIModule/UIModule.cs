...
namespace ZeroEngine
{
    ...
    {
        [SerializeField] private Transform m_InstanceRoot = null; //根节点
        
        [SerializeField] private bool m_dontDestroyUIRoot = true; //不随场景删除

        [SerializeField] private bool m_enableErrorLog = true; //是否在报错时显示日志

        [SerializeField] private Camera m_UICamera = null; //根相机（只显示UI层）

        private readonly List<UIWindow> _stack = new List<UIWindow>(100); //UI堆栈

        public const int LAYER_DEEP = 2000;  //层与层之间Sorting Order间隔
        public const int WINDOW_DEEP = 100;  //层内不同Canvas的Sorting Order间隔
       ...
       /// <summary>
        ...
        private void ShowUIImp(Type type, bool isAsync, params System.Object[] userDatas)
        {
            ...
            if (IsContains(windowName))
            {
                ...
                Pop(window); //弹出窗口
                Push(window); //重新压入
                window.PushInvoke(OnWindowPrepare, userDatas);
            }
            else
            {
