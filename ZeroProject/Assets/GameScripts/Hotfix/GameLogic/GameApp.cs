...
public partial class GameApp:Singleton<GameApp>
{
    ...
    private void Update()
    {
        ZeroProfiler.BeginFirstSample("Update");
        var listLogic = _listLogicMgr;
        var logicCnt = listLogic.Count;
        for (int i = 0; i < logicCnt; i++)
        {
            var logic = listLogic[i];
            ZeroProfiler.BeginSample(logic.GetType().FullName);
            logic.OnUpdate();
            ZeroProfiler.EndSample();
        }
        ZeroProfiler.EndFirstSample();
    }

    private void FixedUpdate()
    {
        ZeroProfiler.BeginFirstSample("FixedUpdate");
        var listLogic = _listLogicMgr;
        var logicCnt = listLogic.Count;
        for (int i = 0; i < logicCnt; i++)
        {
    ...
    {
        ZeroProfiler.BeginFirstSample("LateUpdate");
        var listLogic = _listLogicMgr;
        var logicCnt = listLogic.Count;
        for (int i = 0; i < logicCnt; i++)
        {
